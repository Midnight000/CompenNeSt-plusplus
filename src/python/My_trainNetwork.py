...
def initCompenNeSt(compen_nest, dataset_root, device):
    ckpt_file = '../../checkpoint/init_My_CompenNeSt_l1+ssim_500_24_2000_0.001_0.2_800_0.0001.pth'
...
def trainModel(net, train_data, valid_data, train_option):
    ...
    while iters < train_option['max_iters']:
        ...
        pred = predict(net, dict(cam=cam_train_batch, cam_surf=cam_surf_train_batch))
        prj_train_pred = pred[:,0:3,:,:]
        loss_train_pred = pred[:,3,:,:]
        ...
        if train_option['pre-trained']:
            ...
            if iters <= 200:
                train_loss_batch, train_l2_loss_batch = computeLoss(loss_train_pred, prj_train_pred, prj_train_batch, 'l1')
            else:
                train_loss_batch, train_l2_loss_batch = computeLoss(loss_train_pred, prj_train_pred, prj_train_batch, train_option['loss'])
        else:
            train_loss_batch, train_l2_loss_batch = computeLoss(loss_train_pred, prj_train_pred, prj_train_batch, train_option['loss'])
...
def computeLoss(loss_pred, prj_pred, prj_train, loss_option):
    train_loss = 0
    ...
    if 'loss' in loss_option and 'l2' in loss_option:
        train_loss += l2_fun(l2_loss, loss_pred)
